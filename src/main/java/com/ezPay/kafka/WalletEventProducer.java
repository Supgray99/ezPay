...
public class WalletEventProducer {
    ...
    //to observe retry attempts
    @Bean
    public ProducerListener<String, WalletTransferEvent> kafkaProducerListener() {
    ...
    public void publish(WalletTransferEvent event) {
        ...
        future.whenComplete((result, ex) -> {
            if (ex != null) {
                log.error("Kafka publish FAILED (after retries) for event: {} | Reason: {}",
